...
public class MessageController {
    ...
    public Object InsertMessage(
            @RequestParam(name = "message", required = true) String content,
            HttpServletRequest request) {
        try {
            String ip = request.getLocalAddr();
            JSONObject addresses = AddressUtils.getIp(ip);
            Message msg = new Message();
            msg.setAddTime(new Date());
            msg.setIp(ip);
            msg.setContent(content);
            IpEntity ipEntity = JSON.toJavaObject(addresses, IpEntity.class);
            msg.setAddress(ipEntity.toString());
